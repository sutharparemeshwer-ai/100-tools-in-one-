...
// --- DOM Elements ---
let cityInput, searchBtn, locationBtn, statusEl, weatherDisplayEl, weatherContainer;
...
function getAnimatedIconName(iconCode) {
    ...
    const map = {
        ...
        '09d': 'rainy-6', '09n': 'rainy-6',
        '10d': 'rainy-3', '10n': 'rainy-5',
        '11d': 'thunder', '11n': 'thunder',
        '13d': 'snowy-6', '13n': 'snowy-6',
        '50d': 'mist', '50n': 'mist',
    };
...
async function fetchWeather(url) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(response.status === 404 ? 'City not found.' : `API Error: ${response.statusText}`);
        }
...
export function cleanup() {
    ...
    cityInput.removeEventListener('keypress', (e) => e.key === 'Enter' && handleSearch());
